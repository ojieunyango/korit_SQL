...
### HAVING 은 단독으로 쓸수없다. -> GROUP BY 에 종속되어있음. 
...
SELECT 
c.customer_name, order_id, order_date
FROM customer c 
LEFT JOIN orders o 
ON c.customer_id = o.customer_id;

SELECT o.order_id,p.product_id, p.product_name, oi.quantity, o.order_date, c.customer_name --테이블실행에 보이는 구문
FROM orders o -- orders를 o라고 지정 
LEFT JOIN order_item oi ON o.order_id = oi.order_id  --- order_item 을 oi로 지정하고 orders에 order_id(pk)랑 order_item에 id 랑 같은거 보여줌 
LEFT JOIN product p ON p.product_id = oi.product_id --- product를 p라고 지정하고 product에 product_id(pk)랑 order_item에 product_id 랑 같은거 보여줌
LEFT JOIN customer c ON c.customer_id = o.customer_id; --- customer를  c 라고 지정하고 customer에 customer_id(pk)랑 orders 에 customer_id 랑 같은거 보여줌
...
INSERT INTO customer VALUES  
(1 customer_id(p), '김일', '010-1234-5678','서울특별시 강남구'),----p
(2, '김이', '010-2345-3546','부산광역시 해운대구'),
(3, '김삼', '010-5678-3425','대구광역시 수성구'),
(4, '김사', '010-5646-3545','제주특별자치도 서귀포시'),
(5, '김오', '010-7876-5678','경상남도 양산시'),
(6, '김육', '010-3456-5678','울산광역시 울주군');
(7, '김칠', '010-1351-3579', '미국 뉴욕');

INSERT INTO product VALUES
(101 product_id(p), '무선마우스', 25000),------p
(102, '기계식 키보드', 75000),
(103, 'USB-C 케이블', 10000),
(104, 'HDML 케이블', 12000),
(105, '블루투스 스피커', 60000);
...
INSERT INTO orders VALUES 
(1001order_id(p) , '2025-04-01', 1 customer_id), -------p
(1002, '2025-04-11', 2),
(1003, '2025-04-12', 3),
(1004, '2025-04-13', 1),
(1005, '2025-04-13', 5),
...
INSERT INTO order_item VALUES 
(1001 order_id, 101 prodcut_id , 2 quantity),
(1001, 102, 1),
(1002, 103, 3),
(1003, 104, 1),
(1004, 105, 1),
